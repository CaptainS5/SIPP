...
ylimLow <- -4.2 # range for y axis line; if no individual, 4.2
ylimHigh <- 4.2
...
for (probI in 1:3) {
    ...
    for (conI in 1:2) {
      ...
      indiVelMean <- velData[idxStart+5, 1000:1900]
      indiVelTrace[[(probI-1)*2+conI]] <- data.frame(timePoints, indiVelMean)

      velDataT <- velData[idxStart:idxEnd, 1000:1900] # now plot -200 to 700ms; 700:900 = -500 to 700 ms
...
p <- ggplot(velTrace[[1]], aes(x = timePoints, y = velMean)) +
        ...
        geom_line(data = velTrace[[4]], aes(x = timePoints, y = velMean, colour = "70%"), size = 1, linetype = "solid") + 
        ...
        geom_line(data = velTrace[[6]], aes(x = timePoints, y = velMean, colour = "90%"), size = 1, linetype = "solid") + 
