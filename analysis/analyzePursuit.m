...
%         pursuit.onset = trial.saccades.offsets(onsetSacIdx)+1; % move the onset to the end of the saccade 
        pursuit.onsetType = 1;
...
pursuit.closedLoopDuration = closedLoopFrames(end) - closedLoopFrames(1);

if pursuit.closedLoopDuration <= 50 % closed loop phase too short
    pursuit.gainXexternal = NaN;
    pursuit.gainYexternal = NaN;
    pursuit.gainYaverage = NaN;
    pursuit.gain2Dexternal = NaN;
    pursuit.gain2Daverage = NaN;
    pursuit.dirGainExternal = NaN;
    pursuit.dirClpX = NaN;
    pursuit.dirClpY = NaN;
    pursuit.dirRange = NaN;
    pursuit.dirError = NaN;
    pursuit.disCenterMean = NaN;
    pursuit.travelClpDis = NaN;
    pursuit.targetClpDis = NaN;
else
...
%     pursuit.gainXaverage(pursuit.gainXaverage>2.5) = NaN;
    ...
    pursuit.gainYexternal = nanmean(pursuitGainY(closedLoopFrames));
    pursuit.gainYexternal(pursuit.gainYexternal>2.5) = NaN;
    ...
    pursuit.gain2Dexternal = nanmean(pursuitGain(closedLoopFrames));
    pursuit.gain2Dexternal(pursuit.gain2Dexternal>2.5) = NaN;
    ...
    zScore = zscore(pursuitGain(~isnan(pursuitGain)));
    pursuitGain((zScore > 3 | zScore < -3)) = NaN;
    pursuit.gain2Daverage = nanmean(pursuitGain(closedLoopFrames));
    pursuit.gain2Daverage(pursuit.gain2Daverage>2.5) = NaN;
    ...
    pursuit.dirGainExternal = nanmean(dirGainAll(closedLoopFrames));
    
    % the mean direction of eye velocity (smooth pursuit phase)
    ...
    pursuit.dirRange = nanmax(dir)-nanmin(dir); % the difference between the min and max direction; to identify trials with dramatic direction changes
...
%     
    ...
    pursuit.disCenterMean = nanmean(disCenter(closedLoopFrames));
    
    % mainly used for exclusion criteria, the total 2D distance traveled by
    % the eye during the whole clp duration
    pursuit.travelClpDis = sqrt((trial.eyeX_filt(closedLoopFrames(end))-trial.eyeX_filt(closedLoopFrames(1))).^2 ...
        + (trial.eyeY_filt(closedLoopFrames(end))-trial.eyeY_filt(closedLoopFrames(1))).^2);
    pursuit.targetClpDis = sqrt((trial.target.posX(closedLoopFrames(end))-trial.target.posX(closedLoopFrames(1))).^2 ...
