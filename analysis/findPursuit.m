...
if trial.log.eyeType==0 % fixation
    ...
    % x-value: TIME
    % if trial.stim_onset > pursuitSearchStart
    startTime = trial.stim_onset+pursuitSearchStart;
    % we want to make sure the end point is before the catch up saccade
    endTime = trial.stim_onset+pursuitSearchEnd;
    % else
    ...
    fixationInterval = 300; % chose an interval before stimulus onset that
