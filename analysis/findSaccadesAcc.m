function [onsets, offsets] = findSaccadesAcc(stim_onset, stim_offset, speed, acceleration, jerk, thresholds, log)
% stim_onset, stim_offset, speed, acceleration, jerk, threshold
...
% startFrame = trial.log.trialStart; % fixation onset
% endFrame = trial.log.trialEnd; % basically target offset, right after response 
% speed = trial.eyeDX_filt;
% acceleration = trial.eyeDDX_filt;
% jerk = trial.eyeDDDX;
sThreshold = thresholds(1); % saccades during pursuit
mThreshold = thresholds(2); % microsaccades during fixation
...
binEdges = [1; signSwitchesAcc; length(acceleration)];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % include half of a saccade at the end of the interval; if your time window
% % is larger than the real analysis window, comment this out since the
% % saccades at the end of the analysis phase should then be able to being
% % detected; I use this since the recording stops right after target offset
% % in the speed-accuracy task
% if xor(largePeaks(end-1), largePeaks(end))
%     if largePeaks(end-1)
%         relevantPeaks(end-1:end) = [1; 1];
%     else
%         relevantPeaks(end-1:end) = [0; 1];
%     end
% end
% % just not working perfectly now...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if length(offsetIntervals)<length(onsetIntervals) 
    offsetIntervals = [offsetIntervals; length(relevantPeaksDiff)+1]; % not ending until the end, just make the last interval the end
end
...
%     offsetSearchPoint = binEdges(offsetIntervals(ii)+1)-1; % would not miss the last bit of a large saccade
    ...
    offsetSearchPoint = peakIdx(offsetIntervals(ii))+20; % use this if want to exclude immediate pursuit after a saccade, but may miis the last bit of a large saccade
