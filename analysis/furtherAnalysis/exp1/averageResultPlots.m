    ...
    for varN = plotVarStart:plotVarEnd
        for subN = 1:size(names, 2)
            ...
            for internalConN = 1:size(yMeanDiffSub.(plotVariables{varN}){subN}, 2)
                errorbar(apertureAngles, yMeanDiffSub.(plotVariables{varN}){subN}(:, internalConN), yStdDiffSub.(plotVariables{varN}){subN}(:, internalConN), 'color', colorDotCons(internalConN+1, :))
            end
...
%             xticklabels(apertureAngleNames)
            ...
            for internalConN = 1:size(internalCons, 2)
                errorbar(apertureAngles, yMeanSub.(plotVariables{varN}){subN}(:, internalConN), yStdSub.(plotVariables{varN}){subN}(:, internalConN), 'color', colorDotCons(internalConN, :))
            end
