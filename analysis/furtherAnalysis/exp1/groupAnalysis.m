...
subGroup{1} = find(tempData>=2);
subGroup{2} = find(tempData<-2);
...
if averagePlots
    for varN = plotVarStart:plotVarEnd
        for groupN = 1:2
            ...
            ylabel(['Diff ', plotVariables{varN}])
            if varN>=saccadeVarStart
                saveas(gcf, [saccadeFolder, 'sacDiff_', plotVariables{varN}, '_lineplot_subGroup2', num2str(groupN), '.pdf'])
            elseif varN==1
                saveas(gcf, [perceptFolder, 'diff_', plotVariables{varN}, '_lineplot_subGroup2', num2str(groupN), '.pdf'])
            else
                saveas(gcf, [pursuitFolder, 'pursuitDiff_', plotVariables{varN}, '_lineplot_subGroup2', num2str(groupN), '.pdf'])
            end
