                ...
                for varN = 1:length(checkVariables)
                    ...
                    if strcmp(checkVariables{varN}, 'latency') % needs to calculate from onset
                        ...
                        if internalConN>1 % calculate difference from baseline
                            yMeanDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanmean(onsetT-rdkOnT-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                            yStdDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanstd(onsetT-rdkOnT-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                        end
                    elseif strcmp(checkVariables{varN}, 'dirPerturb') % || strcmp(checkVariables{varN}, 'dirOlp') ...
                        ...
                        if internalConN>1 % calculate difference from baseline
                            yMeanDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanmean(dir-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                            yStdDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanstd(dir-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                        end
                    else
                        if strcmp(checkVariables{varN}, 'response')
                            ...
                            yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN) = nanmean(eyeTrialData.(checkVariables{varN})(subN, idxT));
                            yStdSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN) = nanstd(eyeTrialData.(checkVariables{varN})(subN, idxT));
                            if internalConN>1 % calculate difference from baseline
                                yMeanDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanmean(eyeTrialData.(checkVariables{varN})(subN, idxT)-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                                yStdDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanstd(eyeTrialData.(checkVariables{varN})(subN, idxT)-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                            end
                        elseif varN>=saccadeVarStart
                            ...
                            if internalConN>1 % calculate difference from baseline
                                yMeanDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanmean(eyeTrialData.saccades.(checkVariables{varN})(subN, idxT)-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                                yStdDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanstd(eyeTrialData.saccades.(checkVariables{varN})(subN, idxT)-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                            end
                        else
                            % pursuit parameters
                            ...
                            if internalConN>1 % calculate difference from baseline
                                yMeanDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanmean(eyeTrialData.pursuit.(checkVariables{varN})(subN, idxT)-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                                yStdDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1) = nanstd(eyeTrialData.pursuit.(checkVariables{varN})(subN, idxT)-yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, 1));
                            end
                        end
                    end
                    summaryData.(checkVariables{varN})(count, 1) = yMeanSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN);
                    if internalConN>1 % calculate the difference from baseline
                        summaryDataDiff.(checkVariables{varN})(countDiff, 1) = yMeanDiffSub.(checkVariables{varN}){subN, perturbN}(angleN, internalConN-1);
                    end
                end
                count = count+1;
                if internalConN>1 % calculate the difference from baseline
