...
filtFrequency = eyeData.sampleRate/2;
filtOrder = 2;
...
%% velocity
eyeData.DX = diff(eyeData.X)*eyeData.sampleRate;
eyeData.DY = diff(eyeData.Y)*eyeData.sampleRate;
% also fiter velocity traces
DX_tmp = diff(eyeData.X_filt)*eyeData.sampleRate;
eyeData.DX_filt = filtfilt(c,d,DX_tmp);
DY_tmp = diff(eyeData.Y_filt)*eyeData.sampleRate;
eyeData.DY_filt = filtfilt(c,d,DY_tmp);

%% acceleration
% same procedure for acceleration
DDX_tmp = diff(eyeData.DX_filt)*eyeData.sampleRate;
eyeData.DDX_filt = filtfilt(c,d,DDX_tmp);
DDY_tmp = diff(eyeData.DY_filt)*eyeData.sampleRate;
eyeData.DDY_filt = filtfilt(c,d,DDY_tmp);

%% jerk for detecting saccades and quick phases
eyeData.DDDX = diff(eyeData.DDX_filt)*eyeData.sampleRate;
eyeData.DDDY = diff(eyeData.DDY_filt)*eyeData.sampleRate;
