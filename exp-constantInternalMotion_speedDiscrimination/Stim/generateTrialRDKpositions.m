...
[apertureStartDisX,] = dva2pxl(control.rdkDuration*const.rdk.apertureStandardSpeed/2, ...
    0, screen); % the distance between the starting center point and center of screen
% randomize the trajectory center to randomize the starting position
[jitterDisX, jitterDisY] = dva2pxl(const.startingPositionJitter, 0, screen);
...
rdkControl.apertureCenterPos{1} = [rdkControl.randCenterX-apertureStartDisX rdkControl.randCenterY]; % the initial starting position, depends on moving direction and speed
...
for frameN = 1:rdkFrames-1
    ...
    rdkControl.apertureCenterPos{frameN+1} = [rdkControl.apertureCenterPos{frameN}(1)+moveDistanceAperture, ...
        rdkControl.apertureCenterPos{frameN}(2)];
