...
%         end
    elseif const.startExp==-1
        const.numTrialsPerBlock    = 18; %32*ones(1, 10);
    end
    ...
    const.rdk.duration = [0.25, 0.5]; % display duration of the whole RDK, s
...
%     const.rdk.coh = [1]; % for classical RDKs
    ...
    const.line.length = mean(const.rdk.duration)*const.rdk.apertureSpeed/2; % in dva
